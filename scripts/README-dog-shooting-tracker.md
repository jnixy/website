...
| classify | One `claude-haiku-4-5` call per article (forced tool call). Returns `qualifies` plus structured fields. The system prompt (`CLASSIFY_SYSTEM`) encodes the scope below; `PROMPT_VERSION` is stamped on every row. The model must quote its evidence for the date in `incident_date_source`; **no quote → `incident_date` is blanked** (the model fabricates plausible dates otherwise, which splits incidents). Enum drift (e.g. `circumstance` "domestic call response") is coerced onto the vocab. A row with no state, city, *or* agency is dropped as too thin. |
| dedupe | Candidates are blocked by `state` only — no date window, because a fabricated date on one copy of an incident would put it outside any window from the real-dated copy. One `claude-haiku-4-5` call decides same-incident from agency + location + summary. A match appends the URL to the existing row's `additional_sources`; no new row. |
